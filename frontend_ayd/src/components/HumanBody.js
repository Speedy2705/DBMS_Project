...
const HumanBody = () => {
    const [info, setInfo] = useState({ position: '', x: 0, y: 0, detail: '' });
    ...
    const handleClick = (e) => {
        ...
        if (position) {
            ...
            setInfo({
                ...
                detail: `Detailed information about the ${position}.`,
            });
        }
    };
        ...
        <div className='w-[calc(1300px)] flex justify-end'>
            <div onClick={handleClick} className="w-fit mt-5 h-[calc(550px)] relative">
                <div className="human-body w-52 h-[calc(500px)] bottom-24 relative">
                    ...
                    {info.position && <InfoBox position={info.position} x={info.x - 1100} y={info.y - 250} detail={info.detail} onClose={handleClose} />}
                </div>
            </div>
        </div>
    );
